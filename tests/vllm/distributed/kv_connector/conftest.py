        ...
        def loop():
            ...
            while not stop.is_set():
                if not poller.poll(50):
                    continue
                kind, pp_rank, tp_rank = decoder.decode(sock.recv())
                assert kind == GET_META_MSG, f"unexpected query {kind!r}"
                # Two frames, as upstream's listener answers since 0.26: the
                # payload, then the stamp the caller turns into a clock offset.
                sock.send_multipart(
                    [
                        encoder.encode(payload_for(tp_rank)),
                        encoder.encode(time.perf_counter()),
                    ]
                )
