...
def _will_be_skipped(item) -> bool:
    ...
    started, and `pytest tests/vllm` must keep costing nothing for them."""
    try:
...
def _item_spec(item):
    ...
    from tests.vllm.model_specs import CompileModelSpec
...
def _spawn_for(item) -> ModuleSpawn:
    ...
    from tests.vllm.utils import start_module_in_spawned_process
...
def vllm_runner():
    ...
    from tests.vllm.runners import VllmRunner
...
def async_vllm_runner():
    ...
    vllm_runner)."""
    from tests.vllm.runners import AsyncVllmRunner
...
def hf_runner():
    ...
    from tests.vllm.runners import HfRunner
