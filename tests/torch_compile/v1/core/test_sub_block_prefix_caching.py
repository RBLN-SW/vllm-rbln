    ...
    def test_hybrid_different_block_sizes_partial_match(self):
        ...
        config = KVCacheConfig(
            ...
            kv_cache_groups=[
                KVCacheGroupSpec(
                    ["full_attn_layer"],
                    FullAttentionSpec(full_bs, 1, 1, torch.float32),
                ),
                KVCacheGroupSpec(
                    ["sw_layer"],
                    SlidingWindowSpec(
    ...
    def test_eligibility(self):
        ...
        hybrid_mamba_config = KVCacheConfig(
            ...
            kv_cache_groups=[
                KVCacheGroupSpec(
                    ["full_attn_layer"],
                    FullAttentionSpec(8, 1, 1, torch.float32),
                ),
                KVCacheGroupSpec(
                    ["mamba_layer"],
                    MambaSpec(
...
class TestRBLNScheduler:
    ...
    def _create_scheduler(self, monkeypatch, num_blocks=100, max_model_len=None):
        monkeypatch.setenv("VLLM_RBLN_SUB_BLOCK_SIZE", str(self.SUB_BLOCK_SIZE))
    ...
    def test_scheduler_uses_rbln_kv_cache_manager(self, monkeypatch):
        """When prefix caching is enabled and block_size > sub_block_size,
        the scheduler should use RBLNKVCacheManager."""
        scheduler = self._create_scheduler(monkeypatch)
        assert isinstance(scheduler.kv_cache_manager, RBLNKVCacheManager)

    def test_schedule_returns_rbln_scheduler_output(self, monkeypatch):
    ...
    def test_no_copy_ops_on_first_request(self, monkeypatch):
    ...
    def test_no_copy_ops_on_full_block_match(self, monkeypatch):
    ...
    def test_partial_match_generates_copy_ops(self, monkeypatch):
    ...
    def test_partial_block_reused_after_free(self, monkeypatch):
    ...
    def test_scheduler_no_sub_block_when_equal_sizes(self, monkeypatch):
        """When sub_block_size == block_size, RBLNKVCacheManager should NOT
        be used (sub-block matching is meaningless)."""
        monkeypatch.setenv("VLLM_RBLN_SUB_BLOCK_SIZE", str(self.BLOCK_SIZE))
    ...
    def test_multi_turn_conversation(self, monkeypatch):
        ...
        scheduler = self._create_scheduler(monkeypatch, num_blocks=100)
