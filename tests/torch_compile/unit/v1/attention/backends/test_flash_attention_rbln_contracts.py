...
def test_build_given_missing_batch_pad_raises_assertion_error(
    ...
    with pytest.raises(AssertionError, match="batch_pad is required"):
        builder.build(
            common_prefix_len=0,
            common_attn_metadata=common_attn_metadata,
            num_tokens=torch.tensor([1], dtype=torch.int32),
            positions=torch.tensor([0], dtype=torch.int32),
        )
...
def test_build_given_mixed_prefill_and_decode_requests_raises_assertion_error(
    ...
    with pytest.raises(AssertionError):
        builder.build(
            common_prefix_len=0,
            common_attn_metadata=common_attn_metadata,
            num_tokens=torch.tensor([2, 2], dtype=torch.int32).numpy(),
...
def test_build_given_sliding_window_decode_clamps_cache_lengths_and_generates_masks(
    ...
    metadata = builder.build(
        common_prefix_len=0,
        common_attn_metadata=common_attn_metadata,
        num_tokens=torch.tensor([1], dtype=torch.int32).numpy(),
        positions=torch.tensor([6], dtype=torch.int32),
        batch_pad=2,
    )

    assert metadata.is_prefill is False
    assert metadata.cache_seq_lens[0, 0].item() == 4
    assert metadata.cache_offsets[0, 0].item() == 5
    assert metadata.local_block_tables.shape == (2, 1)
...
def test_flash_attention_decode_impl_given_batch_size_greater_than_one_raises_assertion_error(
    ...
    kv_cache = torch.zeros((2, 1, 1, 1, 4, 32), dtype=torch.float32)
