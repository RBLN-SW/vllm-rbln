...
def create_random_inputs(
    ...
    for _ in range(num_inputs):
        ...
        token_mapping += [lora_id] * input_size[token_length_dim]
        prompt_mapping += [lora_id]

    return inputs, token_mapping, prompt_mapping
...
def set_lora_mask(
    ...
    for input, lora_id in zip(inputs, prompt_mapping):
        ...
        mask = torch.zeros(input_len, max_loras * max_lora_rank, dtype=lora_dtype)
        if lora_id != 0 and (lora_index := id_to_index.index(lora_id)) is not None:
            mask[
                :,
                lora_index * max_lora_rank : lora_index * max_lora_rank + max_lora_rank,
            ] = 1
        masks.append(mask)

    lora_mask = torch.cat(masks)
...
def test_embeddings(dist_init, num_loras, device, vocab_size, stage) -> None:
    ...
    lora_config = LoRAConfig(
        max_loras=max_loras, max_lora_rank=8, lora_dtype=torch.bfloat16
    )
    ...
    for i in range(NUM_RANDOM_SEEDS):
        ...
        lora_dict, _ = populate_loras(
            id_to_index, layer=lora_embedding, layer_weights=embedding.weight.T
        )
        ...
        lora_mapping = LoRAMapping(token_mapping, prompt_mapping, is_prefill=stage)
        punica_wrapper.update_metadata(lora_mapping, id_to_index, max_loras, vocab_size)
        ...
        for input_, lora_id in zip(inputs, prompt_mapping):
            ...
            result = rbln_compile(lora_embedding_ref)(
                input_, embedding, lora.lora_a, lora.lora_b
            )
...
def test_lm_head_logits_processor(
    ...
    def _pretest():
        linear = ParallelLMHead(
            vocab_size, 1024, vocab_size, params_dtype=torch.bfloat16
        )
    ...
    for i in range(NUM_RANDOM_SEEDS):
        ...
        inputs, token_mapping, prompt_mapping = create_random_inputs(
            active_lora_ids=list(lora_dict.keys()),
            num_inputs=1 if stage else 8 * num_loras,
            input_size=(1, 1024),
            input_range=(0, 1),
        ...
        set_sampler_indices_padded(
            prompt_mapping, id_to_index, len(inputs), stage, max_loras, device
        )
        ...
        set_sampler_indices_padded(
            prompt_mapping, id_to_index, len(inputs), False, max_loras, device
        )
        lora_result = rbln_compile(lora_logits_processor._get_logits)(
        ...
        expected_result = rbln_compile(logits_processor._get_logits)(
            hidden_states=torch.cat(inputs),
            lm_head=original_lm_head,
            embedding_bias=None,
        )
...
def test_linear_replicated(dist_init, num_loras, device, stage) -> None:
    ...
    def create_random_linear_replicated_layer():
        linear = ReplicatedLinear(4096, 4096, bias=False, params_dtype=torch.bfloat16)
        linear.weight.data = torch.rand_like(linear.weight.data)
    ...
    def lora_linear_replicated_ref(x, linear, lora_a, lora_b, scaling):
        output = linear(x)[0]
        output += x @ lora_a @ lora_b * scaling
        return output
    ...
    for i in range(NUM_RANDOM_SEEDS):
        ...
        for input_, lora_id in zip(inputs, prompt_mapping):
            lora = lora_dict[lora_id]
            result = rbln_compile(lora_linear_replicated_ref)(
                input_, linear, lora.lora_a.T, lora.lora_b.T, lora.scaling
            )
            expected_results.append(result)
        expected_result = torch.cat(expected_results)
        ...
        lora_result = rbln_compile(lora_linear)(torch.cat(inputs))[0]
        expected_result = rbln_compile(linear)(torch.cat(inputs))[0]
...
@pytest.mark.parametrize(
    "fully_shard", [False]
)  # WithSharedLoRA version is not implemented.
@pytest.mark.parametrize("device", DEVICES)
@pytest.mark.parametrize("stage", STAGES)
def test_linear_parallel(
    ...
    def create_random_linear_parallel_layer():
        if orientation == "row":
            linear = RowParallelLinear(
                4096, 4096, bias=False, params_dtype=torch.bfloat16
            )
            linear.weight.data = torch.rand_like(linear.weight.data)
            lora_linear = (
    ...
    def lora_linear_parallel_ref(x, linear, lora_a, lora_b, scaling):
        output = linear(x)[0]
        output += x @ lora_a @ lora_b * scaling
        return output
    ...
    for i in range(NUM_RANDOM_SEEDS):
        ...
        inputs, index_mapping, prompt_mapping = create_random_inputs(
            active_lora_ids=list(lora_dict.keys()),
            num_inputs=1 if stage else 32 * num_loras,
            input_size=(1, 4096),
            input_range=(0, 1),
            input_type=torch.bfloat16,
            device=device,
        ...
        for input_, lora_id in zip(inputs, prompt_mapping):
            lora = lora_dict[lora_id]
            result = rbln_compile(lora_linear_parallel_ref)(
                input_, linear, lora.lora_a.T, lora.lora_b.T, lora.scaling
            )
            expected_results.append(result)
        expected_result = torch.cat(expected_results)
...
def test_column_parallel_packed(
    ...
    def create_column_parallel_packed_layer():
        if repeats == 2:
            linear = MergedColumnParallelLinear(
                4096, [4096] * repeats, bias=False, params_dtype=torch.bfloat16
            )
            linear.weight.data = torch.rand_like(linear.weight.data)
            lora_linear = (
            ...
            linear = QKVParallelLinear(
                4096, 64, 32, bias=False, params_dtype=torch.bfloat16
            )
            linear.weight.data = torch.rand_like(linear.weight.data)
            lora_linear = (
    ...
    for i in range(NUM_RANDOM_SEEDS):
        ...
        for input_, lora_id in zip(inputs, prompt_mapping):
            ...
            result = rbln_compile(lora_column_parallel_packed_ref)(
                input_, linear, subloras
            )
            expected_results.append(result)
        expected_result = torch.cat(expected_results)
...
@pytest.mark.skip(
    "This test has become invalid as a result of "
    "the `__vocab_parallel_embedding__init__` patch."
)
@pytest.mark.parametrize("tp_size", [1, 2, 4, 8])
@pytest.mark.parametrize(
    "seed", list(range(VOCAB_PARALLEL_EMBEDDING_TEST_NUM_RANDOM_SEEDS))
)
def test_vocab_parallel_embedding_indices(dist_init, tp_size, seed):
    random.seed(seed)
