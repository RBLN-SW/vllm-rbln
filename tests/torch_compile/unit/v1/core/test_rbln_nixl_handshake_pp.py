...
def _encode_payload(
    pp_rank, pp_size, *, engine_id="eng", compat="HASH", layers_per_stage=1
):
...
class _FakeSock:
    ...
    def __init__(self, pp_size, *, engine_id="eng", compat="HASH", layers_per_stage=1):
...
class TestPpHandshakeFanout:
    ...
    def test_handshake_registers_only_overlapping_stages(self):
        ...
        only stage 1 overlaps -> only it is registered for reading, while both
        are still handshaked (agents added, layer names recorded)."""
        w = _make_worker()
        w.local_seen_layer_names = ["layer.1"]  # this decode rank's band
        sock = _FakeSock(pp_size=2)
        _handshake(w, sock)
        assert w.add_remote_agent.call_count == 2
        assert set(w._remote_shard_layer_names["eng"]) == {0, 1}
        # Only the overlapping stage (1) is registered / read.
    ...
    def test_multiple_ratio_registers_k_stages(self):
        ...
        stages' layers, so both are registered and the other two are skipped."""
        w = _make_worker()
        w.local_seen_layer_names = ["layer.0", "layer.1"]  # this rank's band
        sock = _FakeSock(pp_size=4)  # stages advertise layer.0..layer.3
        _handshake(w, sock)
        assert w.add_remote_agent.call_count == 4  # all handshaked
