...
IPC timing, which varies the DP batch composition; because the EP+DP MoE forward
is not batch-composition-invariant, near-tie argmax tokens flip and greedy output
diverges run-to-run. RBLNAsyncScheduler closes this in-scheduler by holding the
first step until request ingestion quiesces (see rbln_scheduler.py). These tests
guard that gate. The full device-level determinism/504 repro lives in
docs/async_overlap_tests/ (needs NPU + gpt-oss-120b DP4+EP).
"""
