...
# WRAPPER_SPECS: (module, op_name, args_builder, output_idx, expected_constexpr_tail)
#   output_idx: position of the output tensor in warmup params (used to verify fp32 cast)
#   expected_tail: derived constexpr values the wrapper must pass to warmup
#     attention/causal: (NUM_HEAD, NUM_GROUP, HEAD_DIM, QUERY_LEN, PARTITION_SIZE, NUM_BATCH, DIM_BLOCK_TABLE)
#     flash*:           (NUM_HEAD, NUM_GROUP, HEAD_DIM, QUERY_LEN, NUM_BATCH, PARTITION_SIZE, MAX_SEQ_LEN, NUM_BLOCK, DIM_BLOCK_TABLE)
#     sliding_window:   (NUM_HEAD, NUM_GROUP, HEAD_DIM, QUERY_LEN, WINDOW_SIZE, NUM_BATCH, NUM_BLOCK, DIM_BLOCK_TABLE)
WRAPPER_SPECS = [
    (
...
def test_triton_wrapper_given_real_inputs_calls_warmup_with_expected_derived_params(
    ...
    # q, k, v must be cast to float32 inside the wrapper
    ...
    # output tensor must also be float32
    assert captured["args"][output_idx].dtype == torch.float32
    # returned output must preserve original input dtype
...
def test_triton_wrapper_preserves_output_dtype(
    monkeypatch,
    module_name: str,
    op_name: str,
    build_args,
    ...
    monkeypatch.setattr(module, "warmup", lambda func, *args: None)
