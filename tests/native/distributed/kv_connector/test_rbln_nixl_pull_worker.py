...
class TestUpstreamReachesTheOverride:
    ...
    def test_send_heartbeats_reaches_the_dial_guard(self, monkeypatch):
        # Upstream hands _ensure_handshake pp_size and notif_agents_only
        # positionally here, so an override taking only the first four dies on
        # the first heartbeat -- where the direct-call tests above stay green.
        w = TestShardReadPath._read_worker(pp_size=2)
        w._remote_agents = {}
        ...
        hb = MagicMock(host="host", port=1234, tp_size=1, pp_size=2, req_ids=["r0"])

        NixlBaseConnectorWorker._send_heartbeats(
            w, MagicMock(heartbeat_by_engine={"eng": hb})
        )

        # Our guard answered for the peer in backoff, so upstream neither
        # dialled it nor sent it a heartbeat.
