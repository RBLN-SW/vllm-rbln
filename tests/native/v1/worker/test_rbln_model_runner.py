...
def _resolved_batch(
    *, num_reqs_padded, query_len, num_tokens_padded, route=BatchRoute.AGREED
    ...
    """What _resolve_batch_descriptor returns, for tests that drive _dummy_run
    without a DP group."""
    return (
        BatchDescriptor(
...
class TestDecideBatchDescriptor:
    ...
    def test_decode_pads_to_bucket(self):
        ...
        batch_desc, route, _across = runner._resolve_batch_descriptor(3, 30)
    ...
    def test_prefill_uses_unpadded(self):
        batch_desc, _route, _across = self._runner(
            is_prefill=True
        )._resolve_batch_descriptor(3, 30)
...
class TestDummyRunPadding:
    ...
    @staticmethod
    def _runner(
        monkeypatch, *, reqs_across_dp, tokens_across_dp=None, specialized=True
    ):
        captured: dict = {}
        ...
        def fake_gather(
            ...
            return DPBatchSnapshot(
                ...
                is_idle=(is_idle,) + (False,) * (len(reqs_across_dp) - 1),
...
class TestDummyRunDraftParticipation:
    ...
    # the draft agrees on its own shape across ranks and that collective counts
    # every rank. The query length is the warmed 1 + num_spec, not the adopted one,
    # or the draft's first pass recompiles.
    NUM_SPEC = 2
    ...
    def test_idle_draft_joins_the_agreement_at_spec_qlen(self, monkeypatch):
        runner, drafter = self._runner(monkeypatch, has_drafter=True)
        runner._dummy_run(1, 1, is_prefill=False, warmup=False)
        # minimal num_reqs (1, padded up by num_reqs_padded), warmed qlen
        # 1 + num_spec, non-prefill; no pad kwarg.
