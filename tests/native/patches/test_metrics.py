...
class TestPhase:
    ...
    def test_phase_follows_the_padded_token_count(
        ...
        monkeypatch.setattr(pm, "_resolve_batch_descriptor", lambda self, *a: padding)
        runner = _Runner(is_prefill=is_prefill, max_num_tokens=128)
        ctx = pm._ctx(runner)
        ctx.start_pass()

        assert pm.resolve_batch_descriptor(runner, 1, 1) == padding
    ...
    def test_phase_is_not_marked_outside_a_pass(self, monkeypatch):
        ...
        monkeypatch.setattr(
            pm, "_resolve_batch_descriptor", lambda self, *a: (_desc(128), None, None)
        )
        runner = _Runner()
        ctx = pm._ctx(runner)

        pm.resolve_batch_descriptor(runner, 1, 1)
...
class TestPassBoundary:
    ...
    def test_whole_pass_records_one_sample_per_section(self, monkeypatch):
        ...
        monkeypatch.setattr(
            pm, "_resolve_batch_descriptor", lambda self, *a: (_desc(64), None, None)
        )
        runner = _Runner()
