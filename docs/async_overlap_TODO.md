...
## TODO 1 — batch>1 parity on a FULL (non-truncated) model  [OPEN]
batch>1 defer is **not yet verified** bit-exact to sync. On the full model DP4+EP batch=4
(tok64, greedy) defer diverges from sync on several prompts and is nondeterministic
run-to-run (near-tie token flips). This reproduces under `RBLN_DYNAMO_ASYNC=1` (async I/O,
inline await, no overlap/deferral), so it stems from running the forward on the async worker
thread rather than from the deferral itself; sync is deterministic run-to-run at batch>1.

batch=1 is the verified, bit-exact-safe config (see TODO 3 — DONE); gate the feature to
batch=1 until batch>1 is proven equivalent.

**batch>1 EOS/stop:** not run (moot until batch>1 is verified).
