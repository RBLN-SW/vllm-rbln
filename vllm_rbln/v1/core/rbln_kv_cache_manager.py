...
class RBLNKVCacheManager(KVCacheManager):
    ...
    def get_computed_blocks(self, request: Request) -> tuple[KVCacheBlocks, int]:
        ...
        max_total = request.num_tokens - 1
        if num_computed_tokens + extra_tokens > max_total:
            capped = max_total - num_computed_tokens
            if capped <= 0:
                return computed_blocks, num_computed_tokens
            num_matched = capped // self.sub_block_size
            if num_matched == 0:
                return computed_blocks, num_computed_tokens
            extra_tokens = num_matched * self.sub_block_size
        ...
        self.block_pool.touch(tuple([gm.src_block] for gm in group_matches))
