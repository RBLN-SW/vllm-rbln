...
class RBLNModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    ...
    def dummy_run(self, num_tokens: int = 1) -> None:
        ...
        dummy_num_scheduled_tokens: dict[str, int] = {}
        # 1-token decode: total_tokens == num_computed_tokens forces the
        # `_add_dummy_requests` helper to schedule exactly one token.
        self._add_dummy_requests(
            requests=dummy_requests,
            num_scheduled_tokens=dummy_num_scheduled_tokens,
            total_tokens=max(num_tokens, 1),
            num_computed_tokens=max(num_tokens, 1),
            num_kv_cache_groups=num_kv_cache_groups,
            sampling_params=sampling_params,
            pooling_params=pooling_params,
        )
        sched_output, cleanup_sched_output = self._make_dummy_scheduler_outputs(
            dummy_requests, dummy_num_scheduled_tokens, num_kv_cache_groups
        )
        self._execute_dummy_requests(
            sched_output, cleanup_sched_output, self.intermediate_tensors
        )
    ...
    def _kv_block_sizes_and_max_num_blocks(
        ...
        return block_sizes, max_num_blocks
        ...
        for group in self._kv_cache_spec_attn_group_iterator():
            ...
            for layer_name in group.layer_names:
                ...
                if isinstance(kv_cache_spec, AttentionSpec):
                    ...
                    # GPU-style: just reinterpret the raw byte buffer as the
                    # backend-native shape. No permute, no typed_base sidecar.
    ...
    def initialize_kv_cache_tensors(
        ...
        if self.use_uniform_kv_cache(self.attn_groups, cache_dtype):
            ...
            raise NotImplementedError(
                ...
                " contract that does not exist after the M12-M14 collapse."
            )
        ...
        # Cross-layer KV cache sharing: alias to the target layer's tensor.
        ...
        if has_kv_transfer_group():
            ...
            def rbln_copy_kv_blocks(
                ...
                assert len(src_block_ids) == len(dst_block_ids), (
                    "src_block_ids and dst_block_ids must be the same length;"
                    f" got src={src_block_ids}, dst={dst_block_ids}"
                )
                for layer_name, src in src_kv_caches.items():
                    dst = dst_kv_caches.get(layer_name)
                    if dst is None:
                        continue
                    # KV layout: leading dim is K/V (size 2) for the current
                    # flash-attention backend; the next dim is num_blocks.
                    block_axis = 1 if src.ndim >= 2 and src.shape[0] == 2 else 0
                    for src_idx, dst_idx in zip(src_block_ids, dst_block_ids):
                        dst.select(block_axis, dst_idx).copy_(
                            src.select(block_axis, src_idx)
                        )
