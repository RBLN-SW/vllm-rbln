...
class AsyncRBLNModelRunnerOutput(AsyncModelRunnerOutput):
    def __init__(
        ...
        if envs.VLLM_RBLN_USE_VLLM_MODEL:
            raise RuntimeError(
                "AsyncRBLNModelRunnerOutput requires CUDA streams which "
                "are unavailable when VLLM_RBLN_USE_VLLM_MODEL is enabled. "
                "Async scheduling is not supported on RBLN device."
            )
...
class RBLNModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    ...
    def _copy_valid_sampled_token_count(
        ...
        if envs.VLLM_RBLN_USE_VLLM_MODEL:
            raise RuntimeError(
                "torch.cuda stream operations are not supported on RBLN device. "
                "_copy_valid_sampled_token_count requires CUDA streams which "
                "are unavailable when VLLM_RBLN_USE_VLLM_MODEL is enabled."
            )
    ...
    def _reshape_kv_cache_tensors(
    ...
    ) -> tuple[dict[str, torch.Tensor], dict[str, KVCacheViewInfo]]:
        """
        ...
        return kv_caches, kv_cache_view_infos
    ...
    def _update_kv_cache_base_bindings(
        ...
        if not kv_cache_view_infos_by_layer:
            ...
            return
        self.kv_cache_bases, self.kv_cache_view_infos = build_kv_cache_base_bindings(
    ...
    def _get_kv_cache_forward_context_kwargs(self) -> dict[str, tuple[torch.Tensor, ...]]:
    ...
    def initialize_kv_cache_tensors(
        ...
        if self.use_uniform_kv_cache(self.attn_groups, cache_dtype):
            ...
            kv_caches, kv_cache_view_infos = self._reshape_kv_cache_tensors(
        ...
        for layer_name, target_layer_name in self.shared_kv_cache_layers.items():
            ...
            kv_cache_raw_tensors[layer_name] = kv_cache_raw_tensors[target_layer_name]
