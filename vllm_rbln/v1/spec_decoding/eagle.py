...
def __custom_init__(
    self: EagleProposer, vllm_config: VllmConfig, device: torch.device, runner=None
):
...
def custom_propose(
    ...
    *,
    ...
    if self.attn_metadata_builder is None:
        attn_metadata_builder = self.runner.attn_groups[0][0].get_metadata_builder()
    else:
    ...
    if is_prefills[0]:
        ...
        target_positions = rbln_utils.pad(target_positions, -1, self.max_num_tokens)
    else:
        input_ids = rbln_utils.pad(input_ids, 0, self.runner.max_num_seqs)
        target_positions = rbln_utils.pad(
            target_positions, -2, self.runner.max_num_seqs
        )
        last_token_indices_2d = rbln_utils.pad(
            last_token_indices_2d, 0, self.runner.max_num_seqs
        )
    ...
    if is_prefills[0]:
        ...
        rows = torch.arange(target_positions.size(0), device=target_positions.device)
