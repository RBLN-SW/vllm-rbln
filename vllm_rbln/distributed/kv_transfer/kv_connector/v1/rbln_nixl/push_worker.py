...
class RblnNixlPushConnectorWorker(RblnNixlWorkerBase, NixlPushConnectorWorker):
    ...
    def _writer_still_pending(
        ...
        spans.append(span)
        return not self._covers(
            spans, self._prompt_blocks(meta.local_physical_block_ids) * per_block
        )
    ...
    def _xfer_blocks_for_req(self, req_id: str, meta: "ReqMeta") -> None:
        ...
        if not peer_ranks:
            ...
            if send is not None and send.released:
                ...
                assert self._sw_ratio is not None, (
        ...
        registered = len(remote_block_ids[self._counted_group(remote_block_ids)])
    ...
    def _stream_window(
        ...
        # are none, which is a block written whole or not at all.
        tail_chunks = 0
        if gpb > 1 and offered_tokens and 0 <= hi < registered and closed < have:
