...
class RBLNW8A16BlockFp8LinearOp:
    """Executes a block-scaled FP8 linear layer on RBLN"""
...
class Fp8LinearMethod(LinearMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if self.block_quant:
            ...
            # Delete weight_scale_inv to avoid confusion with weight_scale.
    ...
    def apply(
        ...
        # NOTE(RBLN): RBLN dequantizes to BF16 and runs a plain GEMM
        ...
        elif weight_scale.dim() == 1 and weight_scale.shape[0] == weight_fp8.shape[0]:
            # Per-row scaling
...
def custom_moe_glu_w8a8_fake(
    ...
    # pre-quant hidden_states dtype). hidden_states here is fp8 and
    # hidden_states_scale is fp32, so neither is the right output dtype.
...
class Fp8MoEMethod(FusedMoEMethodBase):
    def __init__(self, quant_config: Fp8Config, layer: torch.nn.Module):
        ...
        self.use_moe_tokens_mask = envs.VLLM_RBLN_USE_MOE_TOKENS_MASK
        # Select the block-fp8 MoE forward once, so apply() does not re-check the
        # env on every forward. W8A16 (default on RBLN-CR03) dequantizes weights
        # to bf16 in the op; W8A8 also dynamically quantizes the activation.
        self._moe_forward = (
            self._moe_forward_w8a16
            if envs.VLLM_RBLN_USE_W8A16
            else self._moe_forward_w8a8
        )
    ...
    def apply(
        ...
        **kwargs: object,
        ...
        # W8A16 vs W8A8 was resolved once in __init__ (self._moe_forward).
        final_hidden_states = self._moe_forward(
            layer,
            hidden_states,
    ...
    def _moe_forward_w8a16(
        self,
        layer: FusedMoE,
        hidden_states: torch.Tensor,
        masked_routing_weights: torch.Tensor,
        gate_proj_weight: torch.Tensor,
        gate_proj_weight_scale: torch.Tensor,
        up_proj_weight: torch.Tensor,
        up_proj_weight_scale: torch.Tensor,
        down_proj_weight: torch.Tensor,
        down_proj_weight_scale: torch.Tensor,
        moe_compute_dtype: torch.dtype,
        expert_map_const: torch.Tensor | None,
    ) -> torch.Tensor:
        # Weight-only dequant to bf16 in the op; activation stays bf16.
        return torch.ops.rbln_custom_ops.custom_moe_glu_group_dequantize(
            hidden_states,
            gate_proj_weight,
            gate_proj_weight_scale,
            up_proj_weight,
    ...
    def _moe_forward_w8a8(
        ...
        if self.use_moe_tokens_mask:
            tokens_mask = get_tokens_mask(
                hidden_states.shape[0], device=masked_routing_weights.device
            )

        # Dynamically quantize hidden_states to fp8 per (1, block_k) group along
        # the contraction dim; hand both the fp8 tensor and the
        ...
        hidden_states_scale = scale.reshape(hs_shape[0], hs_shape[1] // in_block_size)
        return torch.ops.rbln_custom_ops.custom_moe_glu_w8a8(
            hidden_states,
            hidden_states_scale,
            gate_proj_weight,
            gate_proj_weight_scale,
